...
type Props<T> = {
  ...
  onError?: (error?: unknown) => void; // Added error param to onError
...
export const useGet = <T>({
  ...
  const fetchData = useCallback(
    async (fetchPath?: string, fetchHeaders?: Record<string, string>) => {
      setLoading(true);
      setError(null);
      ...
      try {
        const response = await axios.get(`${API_URL}${currentPath}`, {
